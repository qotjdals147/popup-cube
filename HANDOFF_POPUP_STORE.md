...
> **다음 세션 빠른 시작:** `## 7.0` **「다음 세션 착수 가이드」** → **`§7.48` 최신** → `## 8. Changelog` 최신 항목
...
| AD-073 | **(설계 · User 2026-08-27) 반품·교환 v2 로드맵** — 지금 = **반품지·안내 + 클레임 v1** · v2 = **전용 탭·구조화 신청·입고·환불** · §7.48 | User 2026-08-27 | 2026-08-27 |
...
| | |
|---|---|
| **한 줄 요약** | **§58 #8 P1** · AD-069/072 ✅ · **반품 v2 = AD-073 (설계만 · §7.48)** · PG=게이트 |
| **Git 상태** | `main` **`ef452c3`** push ✅ |
| **User 실기** | AD-069 수정 요청 **OK** · AD-072 UI **OK** |
| **다음 에이전트 1순위** | ① **§58 #8 P1** ② User OK 시 **AD-073 Phase R1** (반품·교환 탭 + 클레임 큐) |
...
**최신** — push **`ef452c3`** · AD-069/072 **User 실기 ✅** · **반품 v2 = AD-073 설계(§7.48)**

| | |
|---|---|
| **다음 우선 (에이전트)** | **§58 #8 P1** · User OK 시 **AD-073 Phase R1** |
| **반품 질문** | **일부 있음** — 반품지·**문의(클레임) v1** · **전용 반품 탭 ❌** → **§7.48** |
| **상세** | **§7.48** · 주문 수정 = **§7.45~§7.47**
...
| **손님 UI** | 주문내역 **「문의하기」** 자유 텍스트 | **「반품·교환 신청」** 가이드 UI · **반품지 안내** 노출 |
...
| **배송 중~구매확정 후** | **문의하기** (v1) → v2 **반품 신청** | 카드 내 문의 → v2 **반품·교환 탭** |
...
**v1 클레임과의 관계:** R1은 **문의를 반품 탭으로 모으기** · R2부터 **「문의하기」→「반품·교환 신청」**으로 **카피·UI 교체** (DB 마이그레이션).

#### 착수 전 User와 맞출 것

1. **단순변심** 허용 여부 · **몇 일 이내** (매장 `exchange_guide`와 동일?)
2. **교환 vs 반품** v2에서 **둘 다** vs **반품만**
3. **가챠 당첨품** 반품 시 **roll 복구** 규칙
4. R1만 먼저 할지 · R2까지 한 번에 할지
...
**Git (이번 세션 UI):** **`ef452c3`** · HANDOFF **`b4cb6ff`** (Expo cmd 규칙)
...
*§53 Last updated: 2026-08-13 (AD-061 PG=런칭 게이트 · P1 선행 · §7.0 갱신)*
