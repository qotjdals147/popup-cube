...
### Git commit & push (User 2026-09-22 — **필수 · 예외 없음**)

> **User:** 의미 있는 작업(코드·HANDOFF·설정)이 끝나면 **항상 바로** `commit` + `push` — **「커밋/푸시 할까요?」 묻지 않음**.
...
| | 규칙 |
|---|---|
| **버튼 표시** | **`[제공자 로고] + 「로그인」`** 만 — ❌ 「Google로 로그인」「카카오로 로그인」 (로고에 **Google** 등이 이미 있으면 **이중 노출**) |
| **i18n** | 공통 키 `login.socialProviderLogin` = **`로그인`** (`apps/mobile/src/i18n/ko.ts`) · 카카오/네이버 추가 시 **동일** |
| **접근성** | 스크린리더용만 **`googleA11y`** 등 **제공자명 포함** (`Google로 로그인`) |
| **UI** | `SocialLoginRow` · 이메일 폼 **아래** · §7.86 |
...
**PG·정산 User FAQ (2026-09-11 확정 · 상세 §7.85):**

| User 질문 | 한 줄 답 |
|---|---|
| 여러 매장 샀을 때 **돈이 어디로?** | 손님 **카드 1번 승인** → 보통 **플랫폼 PG 정산 계좌** → **정산/분할**로 **매장별 점주 통장** |
| **매장 POS** 연결? | **v1 ❌** — 온라인몰 + **점주 PC** 주문·송장 (오프라인 POS 연동 = 별도 과제) |
| **매장 계좌번호** 필요? | **정산용 ✅** — **입점·PG 서브몰 등록** 때 수집 · 손님 결제 화면 ❌ · 타 점주·손님 노출 ❌ |
...
**최신** — **§7.85** PG·정산 FAQ · Google git **`120c9b8`**

| | |
|---|---|
| **다음 우선 (에이전트)** | ① Google 로그아웃×3 · ② Kakao/Naver · PG는 **사업자+가맹 후** |
| **User实기** | §7.84 Google **✅** · Dev Client = **§7.84 Dev Client 4줄** |
| **상세** | **§7.85** (최신) · §7.84 · §7.0 「PG·정산 FAQ」표 |
...
### 7.87 세션 인수인계 — **2026-09-22** (AD-081 · **CEO·쿠폰·플랫폼 HOT SKU 보조금**)

| | |
|---|---|
| **Scope** | **코드 ❌** — CEO·User(CTO) **사업·UX 정책 확정** → HANDOFF (CEO 재질문 시 **§7.87 그대로** 답변) |
| **AD** | AD-081 · §53.9.1 · AD-028(점주 할인·가챠)과 **별 축** |
| **구현** | **⬜** — Wave B mock UX 가능 · **보조금 정산 자동 = PG 후 (AD-061)** |

#### CEO가 원하는 것 — 두 층 (User 2026-09-22 **구조 확정 ✅**)

| 층 | 역할 | 쉬운 말 |
|---|---|---|
| **① 실질 (돈·정산)** | **플랫폼이 3~4% 부담**으로 **매장마다 잘 팔리는 상품 1개**를 **밀어주기** | 에이블리식 **「스타 SKU 추가 할인」** — **매장 전체 할인 ❌** |
| **② 체감 (심리·UX)** | **손님 쿠폰 탭 + 쿠폰** — 「**아싸, 여기 할인하네**」 **생색** | **어차피 할 ①**을 **쿠폰으로 포장** · 탭·쿠폰함 **필수**(CEO 확답) |

**에이전트·CTO:** CEO에게 **「쿠폰 탭 = 마케팅 UX, 실제 할인 원천 = HOT SKU + 플랫폼 보조금」** 한 문장으로 말해도 됨.

#### CEO·User Q&A (확정 답)

| # | 질문 | **확정 답** |
|---|---|---|
| **1** | 전 상품? 매장 전체? | **❌** — **매장마다 진짜 잘 팔리는 상품(대표 1개)** 만 **밀어주는** 느낌 |
| **2** | 점주 정산? | **✅** — 손님은 **할인가**로 결제 · **점주는 할인 전(약정) 기준** · **깎인 3~4% = 플랫폼 보조금(우리 부담)** |
| **3** | 「POP-UP CUBE 추가 4%」 같은 문구? | **❌ 굳이 안 함** — 다른 쇼핑몰처럼 **할인가·인기/특가** 정도 · **누가 냈는지 손님 노출 ❌** |
| **4** | PG 전에 가능? | **UI·쿠폰함·「적용됨」 = PG 전 mock 가능** · **점주에게 보조금 **돈** 맞추기 = PG·분할 정산(AD-061) 후 자동** · 그 전 = 수동/내부 장부 |

#### vs 기존 AD-028 (점주 프로모)

| | **AD-028 (지금)** | **AD-081 (CEO안)** |
|---|---|---|
| **누가 할인 부담** | **점주** (매장 % · 상품 할인 · 가챠) | **플랫폼** (HOT SKU **3~4%**) |
| **범위** | 점주가 설정한 상품·매장 규칙 | **매장당 HOT SKU ~1개** (데이터 TOP1 또는 운영·점주 승인 — **착수 시 AD**) |
| **손님 UI** | 장바구니 혜택 단계 등 | **+ 쿠폰 탭·쿠폰 카드** (이름만 쿠폰처럼) |

#### 손님 UX (구현 시 — AD-081)

| 요소 | 규칙 |
|---|---|
| **마이 › 쿠폰** | 사용 가능 N장 · 카드(예: 「인기 상품 추가 할인」) |
| **받기 / 지급** | 기획전·첫 진입 등 — **내부적으로 HOT SKU 쿠폰 1장**에 매핑 가능 |
| **상품·장바구니** | **할인가** · 「쿠폰 적용」 느낌 · **플랫폼명 부담 문구 ❌** |
| **점주 PC 「쿠폰 관리」** | v1 = **플랫폼 발행 쿠폰** 위주 OK · **점주 자체 쿠폰** = 2차 |

#### 정산·PG (CTO용 · CEO 재질문)

```
손님 결제 = 할인가 (예: 9,600원, 4% 적용)
PG 승인 = 그 금액
점주 정산 = 할인 전 상품가 기준(−수수료 등) − 플랫폼 보조금은 **우리가 별도 보전** (설계안 A — User ✅)
내부 DB = coupon/platform_subsidy · funded_by: platform · sku_id · store_id
```

**AD-066(통합 결제 1번):** 쿠폰이 **한 매장 SKU**에만 걸리면 해당 **`orders` 라인**만 · **장바구니 전체 쿠폰**은 **별도 규칙**(PG 착수 시).

#### CEO에게 그대로 읽어도 되는 한 줄

> 「**쿠폰 탭은 고객이 ‘할인 받는 몰’이라 느끼게 하는 장치**고, **실제로 깎아 주는 건 매장별 잘 팔리는 상품에 우리가 3~4% 메워 주는 구조**예요. **점주 돈은 할인 전 기준**이고, **진짜 정산 자동화는 PG 붙을 때** 맞춥니다.»

#### §53.9 · Wave 연결

| Wave | AD-081 |
|---|---|
| **B (성장)** | 쿠폰 탭 mock · HOT SKU 선정 UI(운영) · checkout 할인 **서버 검증** |
| **C (PG 후)** | **보조금 정산·보전** 자동 |
| **D** | **기획전** + **타겟 메시지**와 **같은 SKU** 묶기 |

#### Expo / 코드

| | |
|---|---|
| **이번 §7.87** | **Expo ❌** — HANDOFF만 |

---

### 7.86 세션 인수인계 — **2026-09-16** (AD-080 · **손님 SNS 로그인 버튼 UX**)

| | |
|---|---|
| **Scope** | `apps/mobile` 로그인 — Google **맨 위 풀버튼 ❌** → 이메일 폼 **아래** · **로고 + 「로그인」** (User: Google 워드마크 + 「Google로 로그인」= **Google 이중** → **「로그인」만**) |
| **Git** | **미커밋** — `login.tsx` · `SocialLoginRow.tsx` · `ko.ts` · `assets/google-wordmark.png` · HANDOFF |
| **AD** | AD-080 · AD-078 |

#### 확정 규칙 (카카오/네이버 추가 시 동일)

| 항목 | 규칙 |
|---|---|
| **표시** | `[제공자 공식 로고]` + **`login.socialProviderLogin` (= 로그인)** |
| **금지** | 버튼 글자에 **Google/카카오/네이버** 반복 |
| **a11y** | `googleA11y` 등 **제공자별** 전체 문구 |
| **컴포넌트** | `apps/mobile/src/components/SocialLoginRow.tsx` |

#### Expo 재시작 여부

| 이번 | Expo | User |
|---|---|---|
| §7.86 | ✅ **`apps/mobile`** — Metro **Ctrl+C** → §7.84 **Dev Client 4줄 `--clear`** | QR 또는 Reload |

#### User 실기

| | |
|---|---|
| **상태** | ⬜ — **Google 워드마크 + 「로그인」** · 탭 시 홈 |

---

### 7.85 세션 인수인계 — **2026-09-11** (AD-066 · **PG·정산 User FAQ**)

| | |
|---|---|
| **Scope** | 코드 변경 **없음** — User **PG·다매장 결제·정산** 구조 질문 → HANDOFF 정리 |
| **Git** | **`120c9b8`** (HANDOFF ref) · **`15dbb4a`** (Google AD-079) · **미커밋** = §7.85 · `apps/mobile/.gitignore` |
| **AD** | AD-066 · AD-061 (PG 게이트) |

#### User 질문 → 확정 답 (에이전트·CS 공통)

**Q1. 여러 매장 담아 결제하면 돈이 어디로 가나?**

| 단계 | 설명 |
|---|---|
| 손님 UX | **결제 1번** (AD-066 · 쿠팡형) |
| 카드 | **승인 1건(총액)** → **PG** |
| 1차 도착 | 보통 **POP-UP CUBE 운영 사업자 PG 정산 계좌** (마켓플레이스 표준) |
| DB | webhook 후 **매장별 `orders` N건** (`place_checkout` → `place_order`) |
| 매장별 돈 | **즉시 A·B 통장 ❌** → **분할 정산(split/서브몰)** 또는 **정산 주기 이체**로 **점주 사업자 계좌** |

**Q2. 매장마다 POS(포스) 연결하나?**

| | |
|---|---|
| **v1** | **❌** — 손님=앱/웹 · 점주=**PC 판매자센터**(주문·발송·송장) |
| 오프라인 카운터 POS 연동 | **범위 밖** · 필요 시 **2차 과제** |

**Q3. 각 매장 계좌번호를 알아야 하나?**

| | |
|---|---|
| **정산** | **✅ 입점·PG 셀러 등록** 때 **점주 정산 계좌** (예금주·은행·계좌) 수집 |
| **손님 결제** | 계좌 입력 **❌** |
| **노출** | 다른 점주·손님 **❌** · RLS·암호화·점주 본인만 |
| **구현** | PG 착수 시 `stores`/입점 폼 · PG 서브몰 ID — **AD-061 후** |
...
손님 [결제 1번]
  → PG (카드 총액 승인)
  → (보통) 플랫폼 PG 정산 계좌
  → webhook → Supabase 매장별 orders
  → 정산/split → 점주 A 계좌 / 점주 B 계좌 / (수수료) 플랫폼
```
...
| 이번 | Expo | User |
|---|---|---|
| §7.85 | ❌ **불필요** | 코드·HANDOFF만 |

#### Repo 잡일

| 파일 | 조치 |
|---|---|
| `apps/mobile/AGENTS.md` · `CLAUDE.md` · `LICENSE` | Expo 설치 잔여물 · **`apps/mobile/.gitignore`에 추가 ✅** (로컬 파일은 삭제해도 됨) |

#### 다음

- [ ] §7.85 HANDOFF + `.gitignore` **commit & push** (User 요청 시)
- [ ] Google Dev Client **로그아웃×3** (§7.84)
- [ ] PG — **사업자 등록 + PG 계약** 전 **코드 착수 금지** (AD-061)
...
| **3b** | **입점** — 점주 **정산 계좌** · (PG) **서브몰/셀러 ID** · 사업자번호 — §7.85 | PG 계약과 동시 |
| **4** | 샌드박스·실결제 테스트 → **런칭** | |

**PG 착수 조건 (전부 충족 전 금지):** ① User **사업자 등록 완료** ② **PG 후보·계약 확정** ③ (권장) P1·실기 대부분 OK.

**User 2026-09-11:** 다매장 결제 시 **돈 → PG→(플랫폼)→점주 계좌 정산** · **POS ❌** · **점주 계좌 = 입점 시** — §7.85.
...
- **Author:** Cursor Agent + User
- **Changed:** `20260922b_fix_get_store_reviews_store_id.sql` · `20260922_owner_review_reply.sql` GRANT · `OwnerReviewsPanel` load error log · HANDOFF §7.88 ISS
- **Notes:** Supabase **✅** · **Expo ❌** · hard refresh / local `npm run dev`
...
- **Notes:** AD-081 정책 · AD-080 로그인 · **Expo ✅** mobile → §7.84 Dev Client 4줄 `--clear`

### 2026-09-16 — 손님 SNS 로그인 버튼 카피 (§7.86 · AD-080)
- **Author:** Cursor Agent + User
- **Changed:** `SocialLoginRow.tsx` · `login.tsx` · `apps/mobile/src/i18n/ko.ts` · `assets/google-wordmark.png` · HANDOFF §0 · AD-080 · §7.86 · §25
- **Notes:** 버튼 = **로고 + 「로그인」** · **Expo ✅** `apps/mobile` → §7.84 Dev Client 4줄 `--clear`

### 2026-09-11 — PG·정산 User FAQ (§7.85 · AD-066)
- **Author:** Cursor Agent + User
- **Changed:** HANDOFF §7.85 · §7.0 PG FAQ 표 · 런칭 게이트 3b
- **Notes:** 다매장 **결제 1번→정산 분할** · **POS v1 ❌** · **입점 정산 계좌** · **Expo ❌** · `apps/mobile/.gitignore` (Expo debris)
...
#### 53.9.1 쿠폰·플랫폼 프로모 (AD-081 · CEO 2026-09-22)

> **상세·CEO Q&A = §7.87** · 에이전트는 CEO·투자자 질문 시 **§7.87 표** 우선 인용.

| 구분 | 정책 |
|---|---|
| **실질** | 매장당 **HOT SKU ~1개** · **플랫폼 3~4%** · **점주 = 할인 전 정산** |
| **체감** | **쿠폰 탭·쿠폰함** 필수 — 「할인하는 몰」 **심리** · 내부 = HOT SKU와 **1:1 매핑** 가능 |
| **손님 노출** | **할인가·쿠폰명·인기/특가** · **「플랫폼이 N% 부담」❌** |
| **vs 점주 쿠폰** | AD-028 = **점주 부담** · AD-081 = **플랫폼 부담** — **둘 다** 장기적으로 공존 가능 |
| **PG** | mock UX **PG 전** · **보조금 보전 정산** **PG 후 (AD-061)** |

*§53 Last updated: 2026-09-22 (AD-081 · §53.9.1 CEO 쿠폰 정책)*
...
| **플랫폼 HOT SKU 보조금 (AD-081)** | **우리(플랫폼)가 3~4%** 깎아 주고 **매장당 잘 팔리는 상품 1개**를 밀어주는 할인. **점주 정산은 할인 전 기준** · 손님에게 「플랫폼 부담」문구 **안 씀** · §7.87 |
| **쿠폰 탭·쿠폰함 (AD-081)** | **심리·생색 UX** — 「아싸 할인」. **실제 할인**은 보통 **HOT SKU 보조금**과 **같은 엔진**을 쿠폰으로 **보이게** 포장 · §7.87 |
| **점주 쿠폰 vs 플랫폼 쿠폰** | **점주** = AD-028 매장 할인·가챠(점주 부담) · **플랫폼** = AD-081 HOT SKU 보조금(우리 부담) · CEO 목록 「쿠폰 관리」= **둘 다** 장기적으로 |
...
### 현재 (한국 출시)
- **기본 언어:** `ko` (한국어)
- **문구 위치:** `apps/web/src/i18n/ko.ts` · 손님 앱 = `apps/mobile/src/i18n/ko.ts`
- **SNS 로그인 버튼 (AD-080):** 표시 = **`socialProviderLogin` → 「로그인」** · 제공자명은 **로고만** · a11y = `googleA11y` 등 · §0 표
