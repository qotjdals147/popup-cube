...
/**
 * 마이페이지 — 배송지 관리 탭 (AD-030).
 * 여러 배송지를 등록하고 각각 별명을 붙일 수 있으며, 기본 배송지를 지정할 수 있음.
 * 결제 화면(CartDrawer)에서 여기서 등록한 배송지를 그대로 선택해서 쓴다.
 */
            ...
            {addresses.map((address) => (
              <div key={address.id} style={styles.card}>
                ...
                <div style={styles.cardActions}>
                  {!address.is_default && (
                    <button style={styles.smallButton} onClick={() => handleSetDefault(address.id)}>
                      {t('mypage.setDefault')}
                    </button>
                  )}
                  <button style={styles.smallButton} onClick={() => openEditForm(address)}>
                    {t('mypage.edit')}
                  </button>
                  <button style={styles.smallButtonDanger} onClick={() => handleDelete(address.id)}>
                    {t('mypage.delete')}
                  </button>
                </div>
              </div>
        ...
        <div style={styles.overlay} onClick={() => setFormOpen(false)}>
          <div style={styles.formPanel} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.formActions}>
              ...
              <button style={styles.saveButton} onClick={handleSubmit} disabled={submitting}>
